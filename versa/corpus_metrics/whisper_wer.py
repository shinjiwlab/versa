...
try:
    ...
    logging.warning("Whisper is not properly installed. Please install following https://github.com/openai/whisper")
    whisper = None
...
def whisper_wer_setup(model_tag="default", beam_size=5, text_cleaner="whisper_basic", use_gpu=True):
    if model_tag == "default":
        model_tag = "large"
    device = "cuda" if use_gpu else "cpu"
    if whisper is None:
        raise RuntimeError("Whisper WER is used for evaluation while openai-whisper is not installed")
...
def whisper_levenshtein_metric(wer_utils, pred_x, ref_text, fs=16000):
    ...
    total = ret["whisper_wer_delete"] + ret["whisper_wer_replace"] + ret["whisper_wer_equal"]
    assert total == len(ref_words), (total, len(ref_words))
    total = ret["whisper_wer_insert"] + ret["whisper_wer_replace"] + ret["whisper_wer_equal"]
    ...
    total = ret["whisper_cer_delete"] + ret["whisper_cer_replace"] + ret["whisper_cer_equal"]
    assert total == len(ref_words), (total, len(ref_words))
    total = ret["whisper_cer_insert"] + ret["whisper_cer_replace"] + ret["whisper_cer_equal"]
    assert total == len(pred_words), (total, len(pred_words))
...
if __name__ == "__main__":
    ...
    print("metrics: {}".format(whisper_levenshtein_metric(wer_utils, a, "test a sentence.", 16000)))