    ...
    def forward_features(self, x):
        ...
        if self.config.enable_tscam:
            ...
            if self.config.htsat_attn_heatmap:
                ...
                attn = ((attn * 0.15) + (attn_max * 0.85 - attn_min)) / (
                    attn_max - attn_min
                )
            ...
            if self.config.htsat_attn_heatmap:
                fpx = interpolate(
                    torch.sigmoid(x).permute(0, 2, 1).contiguous() * attn,
            ...
            fpx = interpolate(
                fpx.permute(0, 2, 1).contiguous(), 8 * self.patch_stride[1]
            )
        ...
        if infer_mode:
            ...
            if self.training:
                ...
                for cur_pos in range(
                    0, (self.freq_ratio - 1) * self.spec_size + 1, self.spec_size
                ):
                    tx = x.clone()
                    tx = self.repeat_wat2img(tx, cur_pos)
                    output_dicts.append(self.forward_features(tx))
                clipwise_output = (
                    torch.zeros_like(output_dicts[0]["clipwise_output"])
                    .float()
                    .to(x.device)
                )
                framewise_output = (
                    torch.zeros_like(output_dicts[0]["framewise_output"])
                    .float()
                    .to(x.device)
                )
            ...
            if x.shape[2] > self.freq_ratio * self.spec_size:
                if self.training:
                    ...
                    latent_output = (
                        torch.zeros_like(output_dicts[0]["latent_output"])
                        .float()
                        .to(x.device)
                    )
                    for d in output_dicts:
