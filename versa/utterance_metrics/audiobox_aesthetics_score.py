...
def audiobox_aesthetics_setup(
    model_path=None, batch_size=1, precision="bf16", cache_dir="versa_cache/audiobox", use_huggingface=True, use_gpu=False
):
    if use_gpu:
        device = "cuda"
    else:
    ...
    if model_path is None:
        if use_huggingface:
            ...
            model_path = os.path.join(cache_dir, audiobox_aesthetics.utils.DEFAULT_CKPT_FNAME)
    ...
    predictor = audiobox_aesthetics.infer.AesWavlmPredictorMultiOutput(checkpoint_pth=model_path, device=device, batch_size=batch_size, precision=precision)
    return predictor


def audiobox_aesthetics_score(model, pred_x, fs):
    output = model.forward_versa([(pred_x, fs)])
    return output
