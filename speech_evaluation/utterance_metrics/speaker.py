...
def speaker_model_setup(model_tag="default", model_path=None, model_config=None, use_gpu=False):
    if use_gpu:
        device = "cuda"
    else:
        device = "cpu"
    if model_path is not None and model_config is not None:
        model = Speech2Embedding(model_file=model_path, train_config=model_config, device=device)
    else:
...
def speaker_metric(model, pred_x, gt_x, fs):
    ...
    embedding_gen = model(pred_x)
    embedding_gt = model(gt_x)
...
if __name__ == "__main__":
    ...
    print("metrics: {}".format(speaker_metric(a, 16000)))