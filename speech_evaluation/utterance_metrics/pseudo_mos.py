    ...
    for predictor in predictor_dict.keys():
        if predictor == "utmos":
            ...
            if fs != predictor_fs["aecmos"]:
                pred_aecmos = librosa.resample(pred, orig_sr=fs, target_sr=predictor_fs["aecmos"])
            else:
                pred_aecmos = pred
            ...
            if fs != predictor_fs["dnsmos"]:
                pred_dnsmos = librosa.resample(pred, orig_sr=fs, target_sr=predictor_fs["dnsmos"])
            else:
                pred_dnsmos = pred
            ...
            if fs != predictor_fs["plcmos"]:
                pred_plcmos = librosa.resample(pred, orig_sr=fs, target_sr=predictor_fs["plcmos"])
            else:
                pred_plcmos = pred
